...
package org.kanban.webui.apps.form;
...
import org.kanban.apps.form.KanbanBoard;
		...
		// Check event ONDoubleCLICK on a cell Navigate into documents
			...
			if (me.getTarget() instanceof Cell) {
				...
				if (!swapCard(startStatus, endStatus, startField))
					Messagebox.show(MKanbanCard.KDB_ErrorMessage); //TODO red1 direct messaging English only
			...
			public void onEvent(Event event) throws Exception {
				...
				if (DialogEvents.ON_BEFORE_RUN_PROCESS.equals(event.getName())) {
					...
					if (processModalDialog.isCancel()) {
						...
						//ProcessInfoDialog.showProcessInfo(m_pi, windowNo, kForm, true);	 -- red1 for faster process
					}
