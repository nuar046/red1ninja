...
public class ModelClassGenerator
{
	 ...
	 */
	public ModelClassGenerator (int AD_Table_ID, String directory, String packageName, String trxName)
	{
		...
		// Header
		String className = createHeader(AD_Table_ID, sb, mandatory, packageName, trxName);

		// Save
		if ( ! directory.endsWith(File.separator) )
			directory += File.separator;
		if (GenerateModule.IsAllNodes) {
			log.info("TEST OF MODEL CLASS - "+directory + className + ".java");
			return;
		}			
	 ...
	 * 	@return class name
	 */
	private String createHeader (int AD_Table_ID, StringBuilder sb, StringBuilder mandatory, String packageName, String trxName)
	{
		String tableName = "";
		int accessLevel = 0;
...
//			 .append("    protected BigDecimal accessLevel = BigDecimal.valueOf(").append(accessLevel).append(");").append(NL)
	...
	{
		Class<?> clazz = ModelInterfaceGenerator.getClass(columnName, displayType, AD_Reference_ID, trxName);
		...
		if (DisplayType.isID(displayType) && !IsKey)
		{
			...
			String referenceClassName = ModelInterfaceGenerator.getReferenceClassName(AD_Table_ID, columnName, displayType, AD_Reference_ID, trxName);
			//
		...
		{
			String staticVar = addListValidation (sb, AD_Reference_ID, columnName, trxName);
		...
		{
			String staticVar = addListValidation (sb, REFERENCE_PAYMENTRULE, columnName, trxName);
			sb.insert(0, staticVar);			
		}
	...
	public static void generateSource(String sourceFolder, String packageName, String entityType, String tableName, String trxName)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				new ModelClassGenerator(rs.getInt(1), directory.toString(), packageName, trxName);
			}
		}
		catch (SQLException e)
		{
