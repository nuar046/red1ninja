...
public class ModelInterfaceGenerator
{
	...
	public ModelInterfaceGenerator(int AD_Table_ID, String directory, String packageName, String trxName) {
		this.packageName = packageName;
		// create column access methods
		StringBuilder mandatory = new StringBuilder();
		StringBuilder sb = createColumns(AD_Table_ID, mandatory, trxName);

		// Header
		String tableName = createHeader(AD_Table_ID, sb, mandatory, trxName);
		...
		if (GenerateModule.IsAllNodes) {
			log.info("TEST OF MODEL INTERFACE - "+directory + tableName + ".java");
			return;
		}			
	 ...
	 * @return class name
	 */
	private String createHeader(int AD_Table_ID, StringBuilder sb, StringBuilder mandatory, String trxName) {
		String tableName = "";
		int accessLevel = 0;
	 ...
	 */
	private StringBuilder createColumns(int AD_Table_ID, StringBuilder mandatory, String trxName) {
...
//				+ " AND c.ColumnName NOT LIKE 'Updated%' "
		...
		try {
			...
			while (rs.next()) {
				...
				sb.append(createColumnMethods(mandatory, columnName,
						...
						Description, virtualColumn, IsEncrypted, IsKey, AD_Table_ID, trxName));
			}
		}
		catch (SQLException e)
		{
	...
	private String createColumnMethods(StringBuilder mandatory,
			...
			boolean virtualColumn, boolean IsEncrypted, boolean IsKey, int AD_Table_ID, String trxName)
	{
		Class<?> clazz = getClass(columnName, displayType, AD_Reference_ID, trxName);
		...
		if (isGenerateModelGetter(columnName) && DisplayType.isID(displayType) && !IsKey)
		{
			...
			String referenceClassName = getReferenceClassName(AD_Table_ID, columnName, displayType, AD_Reference_ID, trxName);
			//
	 ...
	 * @return class
	 */
	public static Class<?> getClass(String columnName, int displayType, int AD_Reference_ID, String trxName)
	{
		// Handle Posted
		// TODO: hardcoded
				...
				&& AD_Reference_ID > 0)
		{
			...
			//
			return getClass(columnName, displayType, AD_Reference_ID, trxName); // recursive call with new parameters
		}
		else
		{
	 ...
	 * @return true if a model getter method (method that is returning referenced PO) should be generated
	 */
	public static boolean isGenerateModelGetterForEntity(int AD_Table_ID, String toEntityType, String trxName)
	{
		final String fromEntityType = DB.getSQLValueString(trxName, "SELECT EntityType FROM AD_Table where AD_Table_ID=?", AD_Table_ID);
	...
	public static String getReferenceClassName(int AD_Table_ID, String columnName, int displayType, int AD_Reference_ID, String trxName)
	{
				...
				|| (displayType == DisplayType.Search && AD_Reference_ID == 0))
		{
			...
			if (table==null){ //newly created table will be null
				table = new MTable(Env.getCtx(),AD_Table_ID,trxName);
			}else{
				...
				if (!isGenerateModelGetterForEntity(AD_Table_ID, entityType, trxName))
				{
	 ...
	 * @param tableLike
	 */
	public static void generateSource(String sourceFolder, String packageName, String entityType, String tableName, String trxName)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				new ModelInterfaceGenerator(rs.getInt(1), directory.toString(), packageName, trxName);
			}
		}
		catch (SQLException e)
		{
